...
flowplayer(function(player, root) {
   ...
   player.removeSubCuepoint=function(index, div){
	   if (!player.cuepoints) player.cuepoints = [];
	   var segment = segmentForCue(index);
	   if (!segments[segment]) segments[segment] = [];
	   if(segment==1||segment==segments.length){
		   var a = segments[segment][0].subtitle.text;
		   segments[segment][0].subtitle.text=a.replace(div,"");
	   }else{
		   var a = segments[segment][1].subtitle.text;
		   segments[segment][1].subtitle.text=a.replace(div,"");
	   }
   }
   player.addSubCuepoint=function(cue){
	   if (!player.cuepoints) player.cuepoints = [];
	   var segment = segmentForCue(cue);
	   if (!segments[segment]) segments[segment] = [];
		 //segment=time-sub[index]//不为1或者最后一个/segments[startTime]
	   if(segment==1||segment==segments.length){
	    	 segments[segment][0].subtitle.text= segments[segment][0].subtitle.text+cue.text;
//		     //cuepoint[i]当中
//		     var i = (segment-1)*2;
//		     console.log(player.cuepoints[i]);
//		     console.log(player.cuepoints[i].subtitle.text);
	     }else{
	    	 var a = segments[segment][1].subtitle.text+cue.text;
		     segments[segment][1].subtitle.text = a;
	     };
...
flowplayer(function(player, root) {
   ...
   // no embedding
	...
	var conf = player.conf,
      ...
      trigger = common.createElement('a', { "class": "fp-embed", title: '标记'});
    
	ui.appendChild(trigger);
	ui.appendChild(common.createElement('div', {className: 'fp-cdiv'},
			'<canvas id = "cccc" class ="fp-canMark" style="position:absolute;cursor:crosshair;"></canvas>'));
	
	var changeCanvas = function(){
		var divH = Math.round($(ui).height()),
		divW = Math.round($(ui).width());
		if(divH/divW<conf.ratio){
			var canvasH = divH;
			var canvasW = divH/conf.ratio;
			var canvasL = (divW - canvasW)/2;
		}else if(divH/divW==conf.ratio){
			var canvasH = divH;
			var canvasW = divW;
			var canvasL = 0;
		};
		var canMark = common.find('#cccc', ui)[0];
		
		canMark.width=canvasW;
		canMark.height=canvasH;
		common.css(canMark,"left",canvasL+'px');
		canMark.getContext("2d").clearRect(0,0,canvasW,canvasH); 
	}
	...
	fptip(root, ".fp-embed", "is-marking");
	
	bean.on(root, 'click', '.fp-embed', function(e,api,root) {
	 //显示画布
		changeCanvas();
		var cdiv = common.find('.fp-cdiv', ui)[0];
		$(".fp-cdiv").toggle();
		if(rootClasses.contains("is-marking")){
			player.pause();
		}else{
			player.play();
		};
		player.trigger("embed", [player]);
   });
...
//   }
   ...
   if (!metaKeyPressed && el.ready) {
      ...
      switch (key) {
         case 38:  el.volume(el.volumeLevel + 0.15); break;        // volume up
         case 40:  el.volume(el.volumeLevel - 0.15); break;        // volume down
         case 39:  el.seeking = true; el.seek(true); break;        // forward
         case 37:  el.seeking = true; el.seek(false); break;       // backward
...
flowplayer.defaults.subtitleParser = function(txt) {
	...
	//获取sublist
	var subObj = JSON.parse(txt);
	 var subList=subObj[0].subtitleUrl;
	  var entries = [];
	  var entry = {};
	  for (var x in subList) {
	    // entry
	    entry = {
	        title: subList[x].title,
	        startTime: subList[x].startTime,
	        endTime: subList[x].endTime,
	        text: subList[x].gps
	    };
	    entries.push(entry);
	  }
...
flowplayer(function(p, root) {
  var wrapClasses, currentPoint, wrap,
      rootClasses = ClassList(root),
      subtitleControl;
  var uiSub = common.find('.fp-ui', root)[0];
  var confR = p.conf.ration;
  ...
  var createUIElements = function() {
    ...
    var divH = Math.round($(playerEl).height()),
		divW = Math.round($(playerEl).width());
	if(divH/divW<0.5625){
		var subH = divH;
		var subW = divH/0.5625;
		var subL = (divW - subW)/2;
	};
	common.css(wrap,"width",subW+'px');
	common.css(wrap,"height",subH+'px');
	common.css(wrap,"left",subL+'px');
  ...
  p.on("fullscreen",function(e){
	//改变subtitle的大小
      common.css(wrap,"width",'100%');
      common.css(wrap,"height",'100%');
      common.css(wrap,"left",'0');
  });
  p.on("fullscreen-exit",function(e){
	//还原subtitle的大小
      var divH = Math.round($(uiSub).height()),
            divW = Math.round($(uiSub).width());
      if(divH/divW<0.5625){
          var subH = divH;
          var subW = divH/0.5625;
          var subL = (divW - subW)/2;
...
flowplayer(function(api, root) {
   ...
   if (conf.adaptiveRatio ) {
 	  var rrr=common.height(root) / common.width(root);
 	  setRatio(rrr);
   }
   ...
   api.on("ready", function(ev, api, video) {
      ...
      if (conf.adaptiveRatio && !isNaN(video.height / video.width)) {
    	  var rrr=common.height(root) / common.width(root);
    	  setRatio(rrr, true);
      }
...
flowplayer(function(e,o){
	...
	l=e.conf,t=flowplayer.common,i=t.createElement,
	d=l.swf.indexOf("flowplayer.org")&&l.e&&o.getAttribute("data-origin"),
	p=d?a(d):t.hostname(),s=(document,l.key);
	...
	if(s&&"function"==typeof key_check&&key_check(s,p)){
		if(l.logo){var c=t.find(".fp-player",o)[0],h=i("a",{className:"fp-logo"});
		d&&(h.href=d),l.embed&&l.embed.popup&&(h.target="_blank");
		var y=i("img",{src:l.logo});
		h.appendChild(y),(c||o).appendChild(h)}l.brand&&d||l.brand&&l.brand.showOnOrigin?f(l.brand.text||l.brand,d||location.href):t.addClass(o,"no-brand")
	}else{
		f("flowplayer","http://flowplayer.org");
		var h=i("a",{href:"http://flowplayer.org"});
		//o.appendChild(h);
		var u=i("div",{className:"fp-context-menu"},'<ul><li class="copyright">&copy; 2015</li><li><a href="http://flowplayer.org">About Flowplayer</a></li><li><a href="http://flowplayer.org/license">GPL based license</a></li></ul>'),
		g=window.location.href.indexOf("localhost"),c=t.find(".fp-player",o)[0];
		7!==g&&(c||o).appendChild(u),e.on("pause resume finish unload ready",
			function(e,a){
				...
				if((4===n||5===n)&&t.addClass(o,"no-brand"),/pause|resume/.test(e.type)&&"flash"!=a.engine.engineName&&4!=n&&5!=n){
					var d={display:"block",position:"absolute",left:"16px",bottom:"46px",zIndex:99999,width:"100px",height:"20px",backgroundImage:"url("+[".png","logo","/",".net",".cloudfront","d32wqyuo10o653","//"].reverse().join("")+")"};
					for(var p in d)d.hasOwnProperty(p)&&(h.style[p]=d[p]);
					a.load.ed=r(h)&&(7===g||u.parentNode==o||u.parentNode==c)&&!t.hasClass(o,"no-brand"),a.load.ed||a.pause()
