...
/***/ function(module, exports, __webpack_require__) {
  ...
  function CustomTime (body, options) {
    ...
    this.customTime1 = new Date(); // Arik
  ...
  CustomTime.prototype._create = function() {
    ...
    drag1.style.top = '0px';
    drag1.style.left = '-10px';
    drag1.style.height = '100%';
    drag1.style.width = '20px';
    bar.appendChild(drag1);
    /***  **/
    ...
    /** Arik **/
    // attach event listeners
    this.hammer1 = Hammer(bar1, {
        prevent_default: true
    });
    this.hammer1.on('dragstart', this._onDragStart1.bind(this));
    this.hammer1.on('drag', this._onDrag1.bind(this));
    this.hammer1.on('dragend', this._onDragEnd1.bind(this));
};
  ...
  CustomTime.prototype.redraw = function () {
    if (this.options.showCustomTime) {
      ...
      if (this.bar.parentNode != parent) {
        ...
        if (this.bar.parentNode) {
          this.bar.parentNode.removeChild(this.bar);
          this.bar1.parentNode.removeChild(this.bar1);  // Arik
      }
        parent.appendChild(this.bar);
        parent.appendChild(this.bar1); // Arik
    }
      ...
      /*** Added by Arik ***/
      var x1 = this.body.util.toScreen(this.customTime1);
      this.bar1.style.left = x1 + 'px';
      this.bar1.title = 'Time: ' + this.customTime;
  }
    else {
      // remove the line from the DOM
      if (this.bar.parentNode) {
    ...
    /*** Added by Arik ***/
    if (this.bar1.parentNode) {
        this.bar1.parentNode.removeChild(this.bar1);
    }
}
...
CustomTime.prototype.getCustomTime = function () {
  ...
  * @param {Date} time
  */
  CustomTime.prototype.setCustomTime1 = function (time) {
      this.customTime1 = new Date(time.valueOf());
      this.redraw();
  };

  /**
  * Retrieve the current custom time 1.
  * @return {Date} customTime
  */
  CustomTime.prototype.getCustomTime1 = function () {
      return new Date(this.customTime1.valueOf());
  };
  ...
  * @private
  */
  CustomTime.prototype._onDragStart1 = function (event) {
      this.eventParams.dragging = true;
      this.eventParams.customTime1 = this.customTime1;
  ...
  * @private
  */
  CustomTime.prototype._onDrag1 = function (event) {
      if (!this.eventParams.dragging) return;

      var deltaX = event.gesture.deltaX,
        x = this.body.util.toScreen(this.eventParams.customTime1) + deltaX,
        time = this.body.util.toTime(x);

      this.setCustomTime1(time);

      // fire a timechange event
      this.body.emitter.emit('timechange1', {
          time: new Date(this.customTime1.valueOf())
      });
  ...
  * @private
  */
  CustomTime.prototype._onDragEnd1 = function (event) {
      if (!this.eventParams.dragging) return;

      // fire a timechanged event
      this.body.emitter.emit('timechanged1', {
          time: new Date(this.customTime1.valueOf())
      });
...
* @param {Date} time
*/
Core.prototype.setCustomTime1 = function (time) {
    if (!this.customTime) {
        throw new Error('Cannot get custom time: Custom time bar is not enabled');
    }

    this.customTime.setCustomTime1(time);
};
...
Core.prototype.getCustomTime = function () {
    ...
    return this.customTime.getCustomTime1();
};
