            ...
            if (!this.manualUpdate) {
                this.$watch('options', (val, oldVal) => {
                    if (!this.chart && val) {
                        ...
                        let notMerge = val !== oldVal;

                        //水球图 notMerge===true会报错
                        if(notMerge){
                            let isLiquid = val.series&&val.series[0]&&val.series[0].type==='liquidFill',
                                isLiquid2 = oldVal.series&&oldVal.series[0]&&oldVal.series[0].type==='liquidFill';
                            if(isLiquid&&isLiquid2){
                                notMerge = false;
                            }
                        }

                        this.chart.setOption(val, notMerge);
                    }
